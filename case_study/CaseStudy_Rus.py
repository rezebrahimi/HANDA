...
target_X_train = np.loadtxt("/CaseStudy_DA/Vectorized/casestudy_DA_Ru_train.txt")
target_y_train = np.loadtxt("/CaseStudy_DA/Vectorized/casestudy_DA_Ru_Y_train.txt")

print "Ru train loaded!"

target_X_test = np.loadtxt("/CaseStudy_DA/Vectorized/casestudy_DA_Ru_test.txt")
target_y_test = np.loadtxt("/CaseStudy_DA/Vectorized/casestudy_DA_Ru_Y_test.txt")

target_y_train=target_y_train.reshape(-1,1)
target_y_test=target_y_test.reshape(-1,1)

print "Ru test loaded!"
...
# This is used in target classifier when there is labeled data
...
def visual_Data_s(model, gpu=True):
    ...
    if gpu:
        ...
        labels = Variable(labels)
    features_aligned = model(inputs)
    #np.savetxt('source_inputs.txt',inputs.cpu().data.numpy())
    #np.savetxt('source_label.txt',labels.cpu().data.numpy(),fmt='%d')
    #np.savetxt('source_feature.txt',features_aligned.cpu().data.numpy())
...
def text_classification_test(model, gpu=True):
    ...
    np.savetxt('casestudy_predict_Ru.txt', outputs.cpu().data,fmt='%1.4f')
    ...
    #print predict.reshape(1, len(inputs))
